...
/*      bTemp[1].x  = cos * distance.x + sin * distance.y;    //  X/r*X + Y/r*Y = x2/r +y2/r
  ...
  */
  ...
  void collisioncurtain (Curtain curtain){
    
    for (int i = 0; i<curtain.positions.length;i++){
      PVector [] distance = new PVector [curtain.positions.length]; 
      distance [i] = PVector.sub(curtain.positions[i],position);
      float [] distancemag = new float [curtain.positions.length]; 
      distancemag[i] =  distance[i].mag();
      float distangle[] = new float [curtain.positions.length]; 
      distangle[i] = distance[i].heading();
      
      if (distancemag[i] < (r+curtain.size)/2){
        System.out.println("true");
        float distanceCorrection = ((r+curtain.size)/2-distancemag[i])/2.0;      // making suret that the balls don't end up inside eachother, doesn't happen often anyway(unless going really fast)
        PVector d = distance[i].copy();
        PVector correctionVector = d.normalize().mult(distanceCorrection);
        curtain.positions[i].add(correctionVector);
        position.sub(correctionVector);
                                                                         // do vector rotation          "x2=cosβx1−sinβy1, y2=sinβx1+cosβy1"
                                                            
        // actual vector calculation starts here
        float sin = sin(distangle[i]);
        float cos = cos(distangle[i]);      
        ...
        PVector[] vFinal = {  
          new PVector(), new PVector()
                                                                                                ...
                                                                                                // final rotated velocity for b[0]
        vFinal[0].x = ((m - curtain.m) * vTemp[0].x)/ (m + curtain.m);
        vFinal[0].y = vTemp[0].y;
